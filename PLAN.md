...
| T-004 | backup-core + GUI/TUI | 进度/事件模型：统一字节口径；核心侧节流或环形缓冲；有界 channel；保留结构事件 | 部分完成（OverallProgress 改为按时间节流（tar.gz/dir 格式下也能刷新）；progress 满时真正丢最旧） |
| T-005 | 设计 + backup-core | retention/history/storage：文件与目录备份统一删除；唯一命名与可靠排序；status 枚举及 cancelled 语义；dir checksum；history/manifest 原子写 | 部分完成（dir 备份重名唯一化已修复） |
| T-006 | backup-core | 路径解析：只对严格匹配的变量语法做平台判定；保留 `%`/`$` 字面量；补边界测试 | 已完成 |
| T-007 | 设计 + backup-cli | CLI 契约：定义 backup 默认行为；apps add/remove 持久化；restore 暴露全部冲突策略；文件日志与退出码 | 进行中（apps add/remove 已持久化到 config；备份实时进度输出；未指定 id 时报 unknown app id；restore 策略与文件日志随 P2） |
